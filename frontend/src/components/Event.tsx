...
const TitleBar = () => (
  <div className="flex items-center gap-4 w-full pt-5 pb-2 ">
    ...
    <div className="text-3xl text-white font-bold">Event Info</div>
  </div>
);
...
const EventInfo: React.FC<{ event: EventClass }> = ({ event }) => {
  ...
  return (
    <div className="bg-blue-300 rounded-[12px] p-5 w-full">
      ...
      <p className="text-sm font-semibold text-gray-500">
        {`${event.UserName} on ${new Date(event.Date_time).toLocaleString(
          "en-IN",
          {
            day: "2-digit",
            month: "short",
...
const EventComment: React.FC<{ data: Comment }> = ({ data }) => {
  ...
  return (
    <div className="flex bg-blue-300 items-center gap-3">
      <FaUserCircle className="h-[60px] w-[60px] text-slate-800" />
